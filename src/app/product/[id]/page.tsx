...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      // Shob product ana checkout list-er jonno
      ...
      // Local storage theke cart data nea
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-24 font-sans">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-40 px-4 h-14 flex items-center justify-between border-b border-gray-100">
        ...
        {/* Top Cart Icon Layout Fixed: opens checkout instantly */}
      ...
      {/* FIXED: Highlighted Badges Section (Cream Box, Black Bold Text) */}
      <div className="grid grid-cols-3 gap-2.5 px-4 py-4 bg-white mb-2">
         <div className="flex flex-col items-center justify-center p-3 border border-[#EADBC8] rounded-2xl bg-[#FFFDF9] shadow-sm">
            <Truck className="h-6 w-6 text-[#5C3A21] mb-1.5" />
            <span className="text-xs font-black text-black text-center leading-tight">Delivery info</span>
         </div>
         <div className="flex flex-col items-center justify-center p-3 border border-[#EADBC8] rounded-2xl bg-[#FFFDF9] shadow-sm">
            <RotateCcw className="h-6 w-6 text-[#5C3A21] mb-1.5" />
            <span className="text-xs font-black text-black text-center leading-tight">Return policy</span>
         </div>
         <div className="flex flex-col items-center justify-center p-3 border border-[#EADBC8] rounded-2xl bg-[#FFFDF9] shadow-sm">
            <ShieldCheck className="h-6 w-6 text-[#5C3A21] mb-1.5" />
            <span className="text-xs font-black text-black text-center leading-tight">Safe pack</span>
         </div>
      </div>
      ...
      <div className="bg-white p-4 mb-2">
        <div className="bg-[#F8F9FA] border border-gray-100 p-4 rounded-2xl flex gap-3 items-start">
           ...
           <div>
             ...
             <p className="text-xs text-gray-600 leading-relaxed">Wrong, missing or damaged item par replacement/refund support available hai. Delivery ke 24 hours ke andar contact karein.</p>
           </div>
        </div>
      </div>
      ...
      <div className="bg-white px-4 py-5 mb-2">
        ...
        <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line mb-6">
          {product.description || "Handpicked premium quality dry fruits selected directly from top farms."}
        </p>
      ...
      <div className="bg-[#F8F9FA] px-4 py-5 mb-2">
        <div className="flex justify-between items-end mb-1">
          ...
          <span className="text-xs font-bold text-[#5C3A21]">View all</span>
        </div>
      ...
      {/* Sticky Bottom Bar with Instant Checkout Sheet Opener */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 shadow-[0_-8px_20px_-3px_rgba(0,0,0,0.08)] z-30 rounded-t-3xl">
        {currentInCart > 0 ? (
          ...
          <div className="flex gap-3">
             <button onClick={() => { updateCart(product.id, 1); setIsCheckoutOpen(true); }} className="flex-1 bg-white border-2 border-[#5C3A21] text-[#5C3A21] font-extrabold py-3.5 rounded-2xl text-sm transition">
               Add to Cart
             </button>
      ...
      {/* ADDED: Full Functional Checkout Drawer for Product Details Page */}
      {isCheckoutOpen && (
          ...
          <div className="bg-[#f8f8f8] w-full rounded-t-[24px] max-h-[85vh] flex flex-col animate-[slideUp_0.3s_ease-out]">
            ...
            {totalCartItems === 0 ? (
              <div className="py-16 flex flex-col items-center justify-center text-center px-4">
                ...
                <button onClick={() => { setIsCheckoutOpen(false); router.push('/'); }} className="bg-[#5C3A21] text-white font-extrabold py-3.5 px-8 rounded-2xl text-sm flex items-center gap-2">
                  ...
                  <div className="space-y-3">
                    {allProducts.filter(p => cart[p.id]).map(p => (
                      <div key={p.id} className="bg-white p-3.5 rounded-2xl flex items-center gap-4 shadow-sm border border-gray-100">
                        ...
                        <div className="bg-[#5C3A21] text-white rounded-xl flex items-center px-2.5 py-1.5">
                           <button onClick={() => updateCart(p.id, -1)} className="text-lg font-medium px-2">−</button>
