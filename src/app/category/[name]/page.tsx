...
import { ShoppingCart, ArrowLeft, Star, X, Tag, ChevronRight } from "lucide-react";
...
export default function CategoryPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      ...
      const { data } = await supabase.from("products").select("*").eq("category", categoryName).order("created_at", { ascending: true });
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20 font-sans">
      <header className="bg-white shadow-sm sticky top-0 z-40 px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3"><button onClick={() => router.push('/')} className="bg-gray-100 p-1.5 rounded-full active:scale-95 transition"><ArrowLeft className="h-5 w-5 text-gray-800" /></button><h1 className="text-lg font-bold text-amber-800 tracking-tight">{categoryName}</h1></div>
        <div onClick={() => setIsCheckoutOpen(true)} className="relative bg-gray-100 p-2 rounded-full cursor-pointer"><ShoppingCart className="h-5 w-5 text-gray-700" />{totalCartItems > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold h-4 w-4 flex items-center justify-center rounded-full border border-white">{totalCartItems}</span>}</div>
      </header>

      <main className="px-4 py-4">
        <div className="flex items-center gap-2 mb-4"><span className="text-amber-600">⚡</span><h3 className="font-bold text-gray-800">Showing all {categoryName}</h3></div>
        <div className="grid grid-cols-2 gap-3">
          {products.length > 0 ? products.map((product) => {
            ...
            return (
              <div key={product.id} onClick={() => router.push(`/product/${product.id}`)} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 flex flex-col cursor-pointer active:scale-[0.98] transition-transform">
                <div className="h-32 bg-gray-50 flex items-center justify-center relative"><span className="absolute top-2 left-2 bg-red-50 text-red-600 text-[9px] font-bold px-1.5 py-0.5 rounded">{product.discount_tag}</span><div className="h-20 w-20 bg-amber-100 rounded-full flex items-center justify-center text-3xl">🌰</div></div>
          ...
          }) : <div className="col-span-2 text-center py-10"><div className="text-4xl mb-2">🛒</div><p className="text-sm text-gray-500 font-medium">No products available in this category yet.</p></div>}
        </div>
      </main>
          ...
          <div className="bg-[#f8f8f8] w-full rounded-t-[24px] max-h-[85vh] flex flex-col animate-[slideUp_0.3s_ease-out]">
            ...
            {totalCartItems === 0 ? (
              <div className="py-16 flex flex-col items-center justify-center text-center px-4">
                ...
                <button onClick={() => { setIsCheckoutOpen(false); router.push('/'); }} className="bg-[#5C3A21] text-white font-extrabold py-3.5 px-8 rounded-2xl text-sm shadow-[0_4px_12px_rgba(92,58,33,0.3)] active:scale-95 transition-transform flex items-center gap-2">Continue Shopping <ChevronRight className="h-4 w-4" /></button>
              </div>
            ) : (
              <>
                  ...
                  <div className="space-y-3">
                    {products.filter(p => cart[p.id]).map(p => (
                      <div key={p.id} className="bg-white p-3.5 rounded-2xl flex items-center gap-4 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.1)]">
                        ...
                        <div className="bg-[#5C3A21] text-white rounded-xl flex items-center px-2.5 py-1.5 shadow-sm"><button onClick={() => updateCartFromCheckout(p.id, -1)} className="text-lg font-medium px-2 active:scale-75 transition-transform">−</button><span className="font-bold text-sm min-w-[20px] text-center">{cart[p.id]}</span><button onClick={() => updateCartFromCheckout(p.id, 1)} className="text-lg font-medium px-2 active:scale-75 transition-transform">+</button></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-white p-4 rounded-t-3xl shadow-[0_-8px_15px_-3px_rgba(0,0,0,0.08)] mt-auto pb-6">
                  ...
                  <button onClick={() => { localStorage.setItem("cartTotal", totalAmount.toString()); router.push(localStorage.getItem("deliveryAddress") ? "/payment" : "/address"); }} className="w-full bg-[#5C3A21] text-white font-bold py-4 rounded-2xl text-lg flex items-center justify-center gap-2 hover:bg-[#4a2e1a] active:scale-[0.98] transition-all shadow-md">Proceed <ChevronRight className="h-5 w-5" /></button>
                </div>
              </>
            )}
          </div>
